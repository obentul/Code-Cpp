...
udpInstance::~udpInstance()
{
	...
	for (size_t i = 0; i < RECV_QUEUE_DEPTH; i++)
	{
		LOG(INFO) << "CLEAR recvQ : " << i;
		char* tmp;
		tmp = recvQ.front();
		if (NULL != tmp){
			free(tmp);
...
unsigned int __stdcall  udpInstance::RecvThread(void* pParam)
{
	...
	while (1) {
		...
		if (nRecv > 0) {
			//LOG(INFO) << "[RecvThread] current recvfrom " << nRecv << " characters";
			//1.对端地址校验(暂不使用)
			//string senderip = inet_ntoa(dstaddr.sin_addr);
			//u_short senderport = ntohs(dstaddr.sin_port);
			//short senderfamily = dstaddr.sin_family;
			//LOG(INFO) << "[RecvThread] dst ip is -> " << senderip;
			//LOG(INFO) << "[RecvThread] dst port is -> " << senderport;
			//if (senderip != pthis->dst_ip || senderport != pthis->dst_port || senderfamily != pthis->dst_addr.sin_family){
			//	LOG(INFO) << "[RecvThread] dst addr does not match to last saved";
			//	continue;
			//}

			//2.数据压入接收队列(如果收到的数据 大于 BYTES_PER_FRAME ，则在队列中分配多个段，保证每一段都是320字节，如果分割到最后 不足 BYTES_PER_FRAME，则使用\0作为空白填充 )
...
int udpInstance::appendToSendQ(char* address, DWORD size)
{
	...
	sendlock();
	sendQ.push(Qbuf);
	//LOG(INFO) << "[appendToSendQ] current send Q depth -> " << sendQ.size();
	sendunlock();
	return 0;
}
...
int udpInstance::startRecvThread() {
	HANDLE threadhandle = (HANDLE)_beginthreadex(NULL, 0, RecvThread, this, 0, &recvThreadID);
...
int udpInstance::startSendThread() {
	HANDLE threadhandle = (HANDLE)_beginthreadex(NULL, 0, SendThread, this, 0, &sendThreadID);
	if (NULL == threadhandle){
