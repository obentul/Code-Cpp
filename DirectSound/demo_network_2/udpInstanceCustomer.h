...
#define AUDIOPLAY_BUFFERSIZE AUDIOPLAY_EVENT_NUM*BYTES_PER_FRAME*FRAMES_PER_WRITE_DS	//播放缓冲区尺寸 320*40*4
#define AUDIOCAPTURE_BUFFERSIZE AUDIOCAPTURE_EVENT_NUM*BYTES_PER_FRAME*FRAMES_PER_READ_DS	//采集缓冲区尺寸 320*40
...
private:
	...
	static unsigned int __stdcall PlayThread_1(void* pParam);		//语音播放线程，从udpinstance的接收队列中取数据，做一定的处理(规整化为可播放的数据)，输出语音到播放设备
