...
unsigned int __stdcall MeshVoice::PlayThread(void* pParam)
{
	...
	while (1){	//注：此循环中 无阻塞点，需要增加sleep来手动阻塞，否则占用cpu
		memset(data, '\0', sizeof(data));
		//每次从队列中读 4 帧数据，如果队列中没有足够的数据，则使用空白帧(\0)填充
		int ret = pthis->readData(data, FRAMES_PER_WRITE_DS);
		if (ret == -1){
			Sleep(100);		//等待数据填充队列，同时0.1s一次进行空白帧填充
			//continue;
		}
...
bool MeshVoice::getCapturedSoundData(DWORD startPos, char *pBuffer, LPDWORD pOutSize)
{
	...
	HRESULT hr = _InputBuffer->Lock(startaddress, BYTES_PER_FRAME * FRAMES_PER_READ_DS,&lpvPtr1, &dwBytes1, &lpvPtr2, &dwBytes2, 0);	//从缓冲区中读数据
...
bool MeshVoice::createSoundCapturedEvent()
{
	...
	for (int i = 0; i < AUDIOCAPTURE_EVENT_NUM; i++)
	{
		PositionNotify[i].dwOffset = BYTES_PER_FRAME * FRAMES_PER_READ_DS * i;	//directsound的缓冲区中，每到达 dwOffset 个偏移量，通知 WaitForMultipleObjects 一次
