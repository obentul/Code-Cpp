...
udpInstance::~udpInstance()
{
	...
	while (sendoff != false || recvoff != false){
		Sleep(10);
	}
	...
	LOG(INFO) << "sMeshVoice Service [SHUTDOWN]  [3/4]  [RecvThread shutdown]";
	...
	LOG(INFO) << "-------------------- sMeshVoice Service [SHUTDOWN]  [4/4]  [SendThread shutdown] --------------------";
...
int udpInstance::udpinit(int dstport, int srcport, string dstip, string srcip)
{
	...
	/*非阻塞socket*/
	int timeout = 200;		//200ms没收到数据，则recvfrom会跳出循环
	int ret1 = setsockopt(sock_fd, SOL_SOCKET, SO_RCVTIMEO, (char*)&timeout, sizeof(timeout));
...
unsigned int __stdcall  udpInstance::SendThread(void* pParam)
{
	...
	LOG(INFO) << "sMeshVoice Service [STARTUP]  [1/4]  [SendThread startup]";

	char tmpbuff[BYTES_PER_FRAME + 1];
	while (false == pthis->sendoff) {
		memset(tmpbuff, '\0', BYTES_PER_FRAME + 1);
		Sleep(20);					//20ms发一次
		/*如果队列为空，则继续判断*/
...
unsigned int __stdcall  udpInstance::RecvThread(void* pParam)
{
	...
	LOG(INFO) << "sMeshVoice Service [STARTUP]  [2/4] [RecvThread startup]";

	while (false == pthis->recvoff) {
		/*如果队列为满，进入消费者自检流程*/
